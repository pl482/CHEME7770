...
let
	...
	for i ∈ 1:ℛ
		flux_table[i,1] = i
		flux_table[i,2] = rna[i]
		flux_table[i,3] = calculated_flux_array_dilute[i]*(1) # units: μmol/gDW-hr
		flux_table[i,4] = expanded_reaction_array[i]
	end
...
begin
	...
	# default value for μ μmol/gDW
	τ=20.0
	μ=log(2)/τ/3600 # units: 1/s
	d=12.4
	V=((4/3)*π*(d/2)^3)*1e-15
	m=1e-9
	f=0.7
	CF=V*(1/m)*(1/(1-f))*(1e6)
	#mol/L *V(HL60/cell)*{mol/cell}*{1cell/WH}*{g/gDW}
	with_terminal() do
