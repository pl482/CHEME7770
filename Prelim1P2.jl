...
md"""
### CHEME 7770 Prelim 1 Problem 2
Pingchuan Liu (PL482)

I use following code to estimate the required paramters for fi
"""
...
begin
	...
	K2 = DSM_parameters[1]
	n2 = DSM_parameters[2]
	ϵ1 = (DSM_parameters[3])/100
	ϵ2 = (DSM_parameters[4])/100
	...
	ϵ_array = [
		0.0 	; # state 1 (just E)
		-ϵ1 	; # state 2 (E bound to S, but no I)
		-ϵ2 	; # state 3 (E bound to I)
	];
	...
	Kₘ = 5 	# units: mM
	kcat = 0.4 # units: s^-1
	ATP=2.3; KATP=0.42;F6P=0.1;KF6P=0.11;
	number_of_steps = 1000

	# substrate range -
	S_array = range(0.0,stop=1,length=number_of_steps) |> collect;
	...
	# let's compute the state-specific factor array -
		f_array = [
			1.0 ; # state 1 
			1.0 ; # state 2
			(F6P/KF6P)*(ATP/KATP)*((S/K2)^(n2))/(1+(S/K2)^(n2))
		];
		...
		θ = p_array[3];
		# compute the rate -
		v_array[i] = (kcat*E)*(F6P/KF6P)*(ATP/KATP)*θ*3600;
	end

	# plot -
	plot(S_array,v_array,xlims=(0.0,0.01, 1),ylims=(0.0,100))
	xlabel!("Effector x (mM)",fontsize=18)
	ylabel!("Rate v (μM/h)",fontsize=18)
	x2=[0; 0.055; 0.093; 0.181; 0.405; 0.99; 1];y2=[3.0; 6.3; 29.8; 52.0; 60.3; 68.7; 68.9]; err=[0.59; 1.20; 5.7; 10.2; 11.8; 13.3; 10];
	scatter!(x2, y2, yerr=err)
	
	
end

# ╔═╡ dee48d81-2587-40d6-9d77-427390415ac7
md"""
## Final chart for part c)
"""

# ╔═╡ 52fb5374-72d3-4adb-8718-309db27b49bd
begin

	# get I -
	K21 = 99.901
	n21 = 0.4
	ϵ11 = 0.19701000000000002
	ϵ21 = 0.14301
	
	# setup system -
	R1 = 8.314 			# units: J/mol-K
	T1 = 273.15 + 25.0 	# units: K
	β1 = 1/R1*T1
	
	# setup energy array -
	ϵ_array1 = [
		0.0 	; # state 1 (just E)
		-ϵ11 	; # state 2 (E bound to S, but no I)
		-ϵ21 	; # state 3 (E bound to I)
	];

	# compute W -
	W_array1 = exp.(-β1*ϵ_array1)

	

	# substrate range -
	S_array1 = range(0.0,stop=1,length=number_of_steps) |> collect;

	# initialize space -
	v_array1 = Array{Float64,1}(undef,number_of_steps)

	# compute the rate -
	for (i,S) ∈ enumerate(S_array1)
	# let's compute the state-specific factor array -
		f_array1 = [
			1.0 ; # state 1 
			1.0 ; # state 2
			(F6P/KF6P)*(ATP/KATP)*((S/K21)^(n21))/(1+(S/K21)^(n21))
		];
	
		# compute the θ variable -
		microstate_array = f_array1.*W_array1;
		Z = sum(microstate_array);
		p_array = (1/Z)*microstate_array;
		θ = p_array[3];
		# compute the rate -
		v_array1[i] = (kcat*E)*(F6P/KF6P)*(ATP/KATP)*θ*3600;
	end

	# plot -
	plot(S_array1,v_array1,xlims=(0.0,0.01, 1),ylims=(0.0,100))
	xlabel!("Effector x (mM)",fontsize=18)
	ylabel!("Rate v (μM/h)",fontsize=18)
	scatter!(x2, y2, yerr=err)
