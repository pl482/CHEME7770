...
md"""
### CHEME 7770 Prelim 1 Problem 3
Pingchuan Liu (PL482)

I use following code to estimate the required paramters
"""
...
@bind DSM_parameters PlutoUI.combine() do Child
	...
	md"""
	X $(
		Child(Slider(0:10:1000000))
	) (mM)  Km $(
		Child(Slider(0.0001:0.001:1))
	) (mM) k1 $(
		Child(Slider(0.001:.1:10000))
	) (mM) k₂ $(
		Child(Slider(0.001:.1:10000))
	) (mM) 
	"""
...
begin
	...
	for (i,S) ∈ enumerate(S_array)
		# compute the rate -
		v_array[i] = X*S/(S+KM)*(k1+k2)/(1+k1+k2);
	end
...
begin
	...
	for (i,S) ∈ enumerate(S_array)
		# compute the rate -
		v_array[i] = X1*S/(S+KM1)*(k11+k21)/(1+k11+k21);
	end
